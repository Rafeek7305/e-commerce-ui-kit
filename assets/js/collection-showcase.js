...
(function($) {
    ...
    var HandzomCategoryShowcase = function($scope, $) {
        var $showcase = $scope.find('.hz-collection-showcase');
        if (!$showcase.length) {
            return;
        }
        ...
        if ($loadMoreBtn.length) {
            $loadMoreBtn.on('click', function(e) {
                ...
                var selectedCats = [];
                $showcase.find('input[name="hz_cat[]"]:checked').each(function() { selectedCats.push($(this).val()); });

                var minPrice = $showcase.find('.hz-cs-price-min').val() || 0;
                var maxPrice = $showcase.find('.hz-cs-price-max').val() || 0;
        ...
        // Category Nested Toggle
        $showcase.find('.hz-cs-cat-toggle').on('click', function(e) {
            e.stopPropagation();
            var $this = $(this);
            var $parent = $this.closest('.hz-cs-category-item');
            var $subList = $parent.find('.hz-cs-subcat-list');
            
            $subList.stop().slideToggle(300, function() {
        ...
        function triggerAjaxLoad() {
            ...
            var selectedCats = [];
            $showcase.find('input[name="hz_cat[]"]:checked').each(function() {
    ...
    $(window).on('elementor/frontend/init', function() {
        elementorFrontend.hooks.addAction('frontend/element_ready/handzom_collection_showcase.default', HandzomCategoryShowcase);
    });
