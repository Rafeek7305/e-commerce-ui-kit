...
class Collection_Showcase extends Widget_Base
{
	public function get_name()
	{
		return 'handzom_collection_showcase';
	}

	public function get_title()
	{
		return esc_html__('Handzom Collection Showcase', 'handzom-ui-kit');
	}
	...
	{
		return ['handzom-collection-showcase'];
	}
	...
	protected function render()
	{
		...
		?>
		<div class="hz-collection-showcase" data-category="<?php echo esc_attr($category_slug); ?>"
				...
				<div class="hz-cs-drawer">
					...
					<div class="hz-cs-drawer-body">
								...
								<div class="hz-cs-filter-list">
									...
									if (!is_wp_error($parent_categories) && !empty($parent_categories)):
										foreach ($parent_categories as $p_cat):
											...
											$has_children = !is_wp_error($child_categories) && !empty($child_categories);
											?>
											<div class="hz-cs-category-item <?php echo $has_children ? 'has-children' : ''; ?>">
												<div class="hz-cs-category-row"
													style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
													<label class="hz-cs-checkbox-label" style="margin-bottom:0;">
														<input type="checkbox" name="hz_cat[]" value="<?php echo esc_attr($p_cat->slug); ?>"
															class="hz-cs-drawer-filter">
														<span><?php echo esc_html($p_cat->name); ?>
															[<?php echo esc_html($p_cat->count); ?>]</span>
													</label>
													<?php if ($has_children): ?>
														<span class="hz-cs-cat-toggle"
															style="cursor:pointer; font-size:18px; line-height:1; user-select:none; color:#999;">+</span>
													<?php endif; ?>
												</div>
												<?php if ($has_children): ?>
													<div class="hz-cs-subcat-list" style="display:none; padding-left:20px; margin-bottom:10px;">
														<?php foreach ($child_categories as $c_cat): ?>
															<label class="hz-cs-checkbox-label" style="margin-bottom:8px;">
																<input type="checkbox" name="hz_cat[]" value="<?php echo esc_attr($c_cat->slug); ?>"
																	class="hz-cs-drawer-filter">
																<span><?php echo esc_html($c_cat->name); ?>
																	[<?php echo esc_html($c_cat->count); ?>]</span>
															</label>
														<?php endforeach; ?>
													</div>
												<?php endif; ?>
						...
						<div class="hz-cs-accordion">
							...
							<div class="hz-cs-accordion-body">
								<div class="hz-cs-price-filter-wrap" style="padding: 10px 0;">
									<div class="hz-cs-price-inputs"
										style="display: flex; gap: 10px; align-items: center; margin-bottom: 15px;">
										<input type="number" name="hz_min_price" class="hz-cs-price-min hz-cs-drawer-filter"
											placeholder="Min" min="0"
											style="width:100%; padding:8px; border:1px solid #ccc; border-radius:4px;">
										<span>-</span>
										<input type="number" name="hz_max_price" class="hz-cs-price-max hz-cs-drawer-filter"
